 ...
 */
bool CCollisionManager::RayBoxCollision(const glm::vec3 vec3BoxMin, 
                                        const glm::vec3 vec3BoxMax,
                                        const glm::vec3 rayOrigin, 
                                        const glm::vec3 rayDirection,
                                        float& t)
{
    float t1 = (vec3BoxMin.x - rayOrigin.x) / rayDirection.x;
    float t2 = (vec3BoxMax.x - rayOrigin.x) / rayDirection.x;
    float t3 = (vec3BoxMin.y - rayOrigin.y) / rayDirection.y;
    float t4 = (vec3BoxMax.y - rayOrigin.y) / rayDirection.y;
    float t5 = (vec3BoxMin.z - rayOrigin.z) / rayDirection.z;
    float t6 = (vec3BoxMax.z - rayOrigin.z) / rayDirection.z;
                                        ...
                                        const glm::vec3 rayEnd)
{
    ...
    float t = FLT_MAX;
    if (RayBoxCollision(vec3BoxMin, vec3BoxMax, rayStart, rayDirection, t) == true)
    {
        /*
        // If you wish to calculate the hit point of this collision
        glm::vec3 hitPoint = rayOrigin + t * rayDirection;
